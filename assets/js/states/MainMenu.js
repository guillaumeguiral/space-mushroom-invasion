...
ChampGame.MainMenu.prototype = {
    ...
    padConnected: function() {
        ...
        this.add.tween(this.padSprite).to({ alpha: 0 }, 300, Phaser.Easing.Quadratic.InOut, true, 0, -1, true);
    },
    ...
    btnHighScoresHandler: function() {
        this.game.stateTransition.to('high-scores');
    },
    ...
    initGUI: function() {
        ...
        this.add.tween(logo.scale).from({ x: 0.6, y: 0.6 }, 1000, Phaser.Easing.Quadratic.Out, true, 500);
        this.add.tween(logo).from({ alpha: 0 }, 1000, Phaser.Easing.Quadratic.Out, true, 500);

        var btn1 = this.createBtn(this.world.centerX, this.world.centerY + 80, "START", this.btnStartHandler);
        var btn2 = this.createBtn(this.world.centerX, this.world.centerY + 140, "HIGH SCORES", this.btnHighScoresHandler);
        var btn3 = this.createBtn(this.world.centerX, this.world.centerY + 200, "HELP", this.btnHelpHandler);

        var btnFullScreen = this.createIconBtn(this.world.width - 48, 48, 'icon-fullscreen', this.btnFullScreenHandler);

        // Tweens
        this.add.tween(btn1).from({ y: btn1.y + 60, alpha: 0 }, 600, Phaser.Easing.Quadratic.Out, true, 0);
        this.add.tween(btn2).from({ y: btn2.y + 60, alpha: 0 }, 600, Phaser.Easing.Quadratic.Out, true, 200);
        this.add.tween(btn3).from({ y: btn3.y + 60, alpha: 0 }, 600, Phaser.Easing.Quadratic.Out, true, 400);

        this.add.tween(btnFullScreen).from({ x: this.world.width + 24 }, 600, Phaser.Easing.Quadratic.Out, true, 600);
    },
